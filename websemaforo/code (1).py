...
def is_conflicting_green(semaforos):
    """Verifica si algún otro semáforo del crucero está en verde."""
    for s in semaforos:
        if s["state"] == 1:  # Verde
            return True
    return False
        ...
        while completed_flag:
            ...
            if not semaforo:
                ...
                new_semaforo = {
                    ...
                    "location": location,
                    "crucero_id": "A1",  # Identificador de crucero
                    "mode": 1,
                    "state": 1,
                    "change_flag": 0,
                    "count_cycle": 0,
            ...
            # Obtener otros semáforos del mismo crucero
            crucero_id = semaforo["crucero_id"]
            otros_semaforos = collection.find({"crucero_id": crucero_id, "serial": {"$ne": SERIAL_NUMBER}})

            # Validar que no haya conflicto de semáforos en verde
            if is_conflicting_green(otros_semaforos) and semaforo["state"] == 1:
                print("Conflicto detectado: Otro semáforo del crucero está en verde.")
                continue  # No cambiar si hay conflicto
