...
if __name__ == "__main__":
    ...
    try:
        while completed_flag:
            ...
            def can_turn_green(semaforo):
                # Encuentra semáforos en el mismo cruce con prioridad distinta y estado verde
                conflicting_lights = collection.find({
                    "zone": semaforo["zone"],
                    "state": 1,  # Verde
                    "priority": {"$ne": semaforo["priority"]}
                })
                return conflicting_lights.count() == 0
            ...
            # Modo automático modificado
            elif semaforo["mode"] == 1:
                if semaforo["state"] == 1:  # Verde
                    # Realizar parpadeo antes de cambiar a amarillo
                    ...
                    # Verifica si puede cambiar a verde sin conflicto
                    if semaforo["count_cycle"] >= 5 and can_turn_green(semaforo):
                        collection.update_one(
                            {"serial": SERIAL_NUMBER},
